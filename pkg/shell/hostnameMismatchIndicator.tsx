    ...
    useEffect(() => {
        cockpit.spawn(["systemctl", "is-active", "iotedge-docker"], { superuser: "try" }).then((result) => {
            ...
            if (error && typeof error === "object" && "message" in error) {
                console.error("Failed to check device onboarded status:", error.message);
            } else {
                console.error("Failed to check device onboarded status:", error);
            }
        });
    }, [state]);
